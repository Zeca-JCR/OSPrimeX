...
interface OSWindowManagerProps {
    sidebarCollapsed: boolean;
}

const OSWindowManager = ({ sidebarCollapsed }: OSWindowManagerProps) => {
    const { windows, activeWindowId, minimizeOS, focusOS, closeOS } = useOS();
    const location = useLocation();

    // Auto-minimize windows when navigating to non-OS routes
    useEffect(() => {
    ...
    }, [location.pathname, activeWindowId, minimizeOS]);
                    ...
                    return (
                        ...
                        >
                            <DetalhesOS
                                osId={win.id}
                                isWindowMode={true}
                                onClose={() => closeOS(win.id)}
                                onMinimize={() => minimizeOS(win.id)}
                            />
                        </div>
                    );
                })}
            ...
            <div className="fixed bottom-4 right-4 z-[1010] flex items-end gap-2 px-4 py-2 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 transition-all duration-300">
                {windows.map((win) => (
                    <button
                        ...
                        `}
                        title={(win.title as string) || `OS #${(win.numero as number) || win.id}`}
                    >
                        ...
                        {(win as { isDirty?: boolean }).isDirty && (
                        ...
                        <div className="absolute -top-10 scale-0 group-hover:scale-100 transition-transform bg-black text-white text-xs px-2 py-1 rounded shadow-md whitespace-nowrap">
                            {(win.title as string) || `OS #${(win.numero as number) || win.id.toString().slice(-4)}`}
                            {(win as { isDirty?: boolean }).isDirty && <span className="ml-1 text-amber-400">(Não salvo)</span>}
                        </div>
                    </button>
                ))}
            </div>
