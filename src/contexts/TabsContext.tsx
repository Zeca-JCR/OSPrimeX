...
export const TabsProvider = ({ children }: TabsProviderProps) => {
    ...
    const openTab = useCallback((tab: { id: string; type: string; title: string; data?: Record<string, unknown> }) => {
        setTabs(prev => {
    ...
    const updateTab = useCallback((tabId: string, data: Partial<Tab>) => {
    ...
    const registerSaveHandler = useCallback((tabId: string, saveFunction: SaveHandler) => {
    ...
    const unregisterSaveHandler = useCallback((tabId: string) => {
        setSaveHandlers(prev => {
            ...
            delete newHandlers[tabId];
            return newHandlers;
        });
    }, []);

    // Obter função de salvar de uma aba
    const getSaveHandler = useCallback((tabId: string): SaveHandler | null => {
